...
var eventLabelDelimiter = elements.getAttr(document.documentElement, "data-jtr-event-label-delimiter") || " ";
...
function page(name, properties) {
  ...
  var event = {
    'type': 'page',
    'name': ["open", name, "page"].join(eventLabelDelimiter),
    'properties': props
  };
  pushEvent(event);
}
    ...
    for(var i = 0; i < path.length; i++) {
      ...
      if(includes.includes(tagname) && !excludes.includes(tagname)) {
        ...
        if(label) {
          ...
          var eventDesc = [eventName, label, elements.role(element)].join(eventLabelDelimiter);
          track(eventDesc);
          break;
        }
      }
